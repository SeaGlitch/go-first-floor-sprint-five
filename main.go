...
const (
	...
	MinsInHour = 60   // количество минут в одном часе
...
type InfoMessage struct {
	...
	Speed        float64       // средняя скорость, с которой двигался пользователь (км/ч)
...
func (t Training) TrainingInfo() InfoMessage {
	...
	info := InfoMessage{
		...
		Distance:     t.distance(),   // расстояние, которое преодолел пользователь (км)
		Speed:        t.meanSpeed(),  // средняя скорость, с которой двигался пользователь (км/ч)
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	// !!!для ревьюера: в предыдущем проекте (спринт 4) вместо "+ CaloriesMeanSpeedShift)" в формуле было "* CaloriesMeanSpeedShift)"
...
func (w Walking) Calories() float64 {
	...
	speedMINS := w.meanSpeed() * KmHInMsec // скорость в м/с
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMINS, 2)/(w.Height/CmInM))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinsInHour)
}
